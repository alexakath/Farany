...
function ListeSalGenJour() {
    ...
    const handlePopupSubmit = async (formData) => {
        ...
        console.log("pas la", filteredIds, formData.mois, formData.annee);

        if (filteredIds.length === 0 || !formData.montant || !formData.majoration || !formData.mois || !formData.annee) {
            setError("Veuillez remplir tous les champs obligatoires et filtrer au moins un salarié.");
            return;
        }

        const montantParJour = Number(formData.montant);
        const majoration = Number(formData.majoration);
        if (!Number.isFinite(montantParJour) || !Number.isFinite(majoration) || montantParJour <= 0 || majoration <= 0) {
            setError("Montant par jour et majoration doivent être des nombres valides supérieurs à 0.");
            return;
        }
        ...
        try {
            const missingDate = await SalariesService.getJourPasSalaire(filteredIds, formData.mois, formData.annee);
            const nombre_missing = missingDate.length;

            if (nombre_missing === 0) {
                setError("Aucun jour sans salaire trouvé pour cette période.");
                return;
            }
            const dated = missingDate[0];
            const datef = missingDate[nombre_missing - 1];
            console.log("miss", missingDate, dated, datef);

            const check = await SalariesService.checkFerie(dated, datef);
            console.log("check", check);

            const nombreJoursFeries = Math.min(Number(check?.nombreDeJours || 0), nombre_missing);
            const nombreJoursNormaux = Math.max(nombre_missing - nombreJoursFeries, 0);
            const total_montant = Number(
                (
                    (montantParJour * nombreJoursNormaux) +
                    (montantParJour * majoration * nombreJoursFeries)
                ).toFixed(2)
            );
            ...
            const requests = filteredIds.map(async (id) => {
                const salaryPayload = {
                    label: `Période du ${missingDate[0]} au ${missingDate[nombre_missing - 1]}`,
                    fk_user: Number(id),
                    datesp: toUnixTimestamp(missingDate[0]),
                    dateep: toUnixTimestamp(missingDate[nombre_missing - 1]),
                    datep: toUnixTimestamp(missingDate[0]),
                    salary: total_montant,
                    amount: total_montant,
                };
...
export default ListeSalGenJour;